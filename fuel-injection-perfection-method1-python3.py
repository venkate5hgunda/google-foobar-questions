...
def convert_to_divisible_by_four(num): # convert into 4 divisible number by adding/subtracting a number
    ...
    if(len(num)<2):
        ...
        if((temp+1)%4==0):
            ans=chr((math.floor(((ord(num[idx])-ord('0'))+1)%10))+ord('0'))
            carry=((ord(num[idx])-ord('0'))+1)//10
            idx-=1
            while(idx>=0 or carry!=0):
                if(idx<0):
                    ans=(chr((math.floor(carry%10))+ord('0')))+ans
                    carry=0
                else:
                    ans=(chr((math.floor(((ord(num[idx])-ord('0'))+carry)%10))+ord('0')))+ans
...
def divide(num,div):
    ...
    ans=""
    while(i<len(num)):
        ans+=chr(math.floor(temp/div)+ord('0'))
